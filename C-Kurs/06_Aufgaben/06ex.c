...
//float n = (float)value / (float)max;
return (float)value / (float)max;
}
Canvas swatch_red_green(Canvas c) {
    int u,v;
    for (v = 0; v < canvas_height(c); v++){
        for (u = 0; u < canvas_width(c); u++){
            canvas_set_g(c,u,v,normalize(v,canvas_height(c)-1));
            canvas_set_r(c,u,v,normalize(u,canvas_width(c)-1));
    }}
    return c;
}
...
Canvas swatch_rgb(Canvas c, ColorChannel channel_x, ColorChannel channel_y) {
    ...
    for (v = 0; v < canvas_height(c); v++){
        for (u = 0; u < canvas_width(c); u++){
            ...
            canvas_set_channel(c,u,v,channel_x,normalize(u,canvas_height(c)-1));
            canvas_set_channel(c,u,v,channel_y,normalize(v,canvas_width(c)-1));
    }}
    return c;
}
...
RGB rgb_to_gray(RGB color) {
    ...
    g = color.g;
    b = color.b;
    res = (r * 0.2989) + (g * 0.587) + (b * 0.1140);

    RGB color_in_grey;
    color_in_grey.r = res;
    color_in_grey.g = res;
    color_in_grey.b = res;
    return color_in_grey;
}
...
Canvas canvas_to_gray(Canvas c) {
    ...
    for (v = 0; v < canvas_height(c); v++){
        for (u = 0; u < canvas_width(c); u++){
            RGB rgb;
            rgb.r = canvas_get_r(c,u,v);
            rgb.g = canvas_get_g(c,u,v);
            rgb.b = canvas_get_b(c,u,v);
            canvas_set_rgb_struct(c,u,v,rgb_to_gray(rgb));
