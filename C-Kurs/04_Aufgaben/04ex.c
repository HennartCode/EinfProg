...
Canvas draw_odd_circle(Canvas c, int x, int y, int radius_from_middle) {
    ...
    for (u = 0; u < w; u++){
        for (v = 0; v < h; v++){
            if (distance(u,v,x,y) <= radius_from_middle){
                canvas_set_black(c,u,v);
            }
        }
    }
    return c;
...
Tipp:u Finden Sie die größte natürliche Zahl, deren Qadrat kleiner oder gleich `n` ist. Es muss nicht effizient sein, wir
...
int my_squareroot(int n) {
    ...
    //int SQRT, step = n;
    // while (1){
    //     if ((((SQRT*SQRT) < n) && ((SQRT+1)*(SQRT+1) > n)) || (SQRT*SQRT == n)){
    //         return SQRT;
    //     }else if (SQRT*SQRT < n && (SQRT+1)*(SQRT+1) < n){
    //         step /= 2;
    //         SQRT += 1;
    //     }else{
    //         step /= 2;
...
Canvas draw_circle(Canvas c, int x, int y, int diameter) {
    int r = diameter/2;
    if (diameter%2 != 0){
        draw_odd_circle(c,x+r,y+r,r);
    }else{
        draw_odd_circle(c,x+r-1,y+r-1,r-1);
        draw_odd_circle(c,x+r-1,y+r,r-1);
        draw_odd_circle(c,x+r,y+r-1,r-1);
        draw_odd_circle(c,x+r,y+r,r-1);
