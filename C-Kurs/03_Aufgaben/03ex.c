...
Canvas descending_diagonal(Canvas c) {
    ...
    int v;
    int i;
    for (v = canvas_height(c)-1; v >= 0; v--){
        for (i = 0; i <= u; i++){
            c = canvas_set_black(c,i,v);
        }
        if (u < canvas_width(c)-1){
        u++;
        }            
    }
    return c;
...
Canvas draw_rectangle(Canvas c, int x, int y, int width, int height) {
    ...
    for (u = x; u <= x + width-1; u++){
        for (v = y; v > (y - height); v--){
            if (0 <= u && u < canvas_width(c) && 0 <= v && v < canvas_height(c)){
