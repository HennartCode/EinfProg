...
int32_t frumpleflabblefizzledipp(int32_t x) {
    int32_t x_ = x;
    int32_t *x_address = &x;
    fizzledipp(x_address);
    int32_t twentsvn = 27; 
    frumple(flabble(&x_, x_address),&twentsvn,x_address);
    return *x_address;
...
int32_t count_fruit_layers(PileOfPancakes p) {
    PileOfPancakes *nextlayer = p.further_layers;
    int layers = 0;
    if (p.layer == Fruit){
        layers++;
    }
    while (nextlayer != NULL){
        PileOfPancakes current_layer = *nextlayer;
        if (current_layer.layer == Fruit){
            layers++;
        }
        nextlayer = current_layer.further_layers;
    }
    return layers;
}
...
bool is_stable(PileOfPancakes p) {
    ...
    while (nextlayer != NULL){
        PileOfPancakes current_layer = *nextlayer;
        stable = (current_layer.layer == ActualPancake);
        nextlayer = current_layer.further_layers;
    }
    return stable;
}
